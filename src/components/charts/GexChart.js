...
const generateDataWithExpirations = (dte, strikes) => {
  ...
  for (let i = 0; i < strikes; i++) {
    ...
    // Add data for each expiration date with GEX-appropriate values
    expirationDates.forEach(exp => {
      const callKey = `calls_${exp}`;
      const putKey = `puts_${exp}`;
      // Mock data generation with larger values for GEX
      dataPoint[callKey] = Math.random() * 400 - 200;
      dataPoint[putKey] = -(Math.random() * 400 - 200);
    });
